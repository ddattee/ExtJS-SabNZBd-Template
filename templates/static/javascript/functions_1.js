...
function nzbretry(nzbid){
    Ext.Ajax.request({
        ...
        success: function(response){
                loadhistory(false);
                Ext.getCmp('HistoryGrid').getSelectionModel().clearSelections()
                infos("Retrying to download item " + nzbid);
        }
    });
}
...
function loadqueue(reload) {
    Ext.Ajax.request({
            ...
            success: function(response){
                    ...
                    for (var i=0;i<o.queue.slots.length;i++){

                            if (o.queue.slots[i].status == 'Paused'){
                                    buttons = '<img style="cursor: pointer;" onclick="queueresume(\''+o.queue.slots[i].nzo_id+'\')" src="../static/images/play.png">';
                            } else {
                                    buttons = '<img style="cursor: pointer;" onclick="queuepause(\''+o.queue.slots[i].nzo_id+'\')" src="../static/images/pause.png">';
                            }
                            buttons += ' ';
                            buttons += '<img style="cursor: pointer;" onclick="queuedelete(\''+o.queue.slots[i].nzo_id+'\')" src="../static/images/delete.png">';
                            buttons += ' ';

                            var tmpRec = new Ext.data.Record();
                            //buttons += '<img style="cursor: pointer;" onclick="queueinfo(\''+o.queue.slots[count].nzo_id+'\')" src="../static/images/info.png">';
                            tmpRec.set('buttons',buttons);
                            tmpRec.set('nzo_id',o.queue.slots[i].nzo_id);
                            tmpRec.set('filename',o.queue.slots[i].filename);
                            tmpRec.set('timeleft',o.queue.slots[i].timeleft);
                            tmpRec.set('size',o.queue.slots[i].sizeleft.substring(0,o.queue.slots[i].sizeleft.length-3)+'/'+o.queue.slots[i].size+' ('+o.queue.slots[i].percentage+' %)');
                            tmpRec.set('percentage','<div style="height:11px;border:1px solid #666666"><div style="height:11px;font-size:9px;background:#666666;width:'+o.queue.slots[i].percentage+'%" class="dl" rel="' + i + '"></div></div>');
                            tmpRec.set('avg_age',o.queue.slots[i].avg_age);
                            tmpRec.set('cat',o.queue.slots[i].cat);
                            tmpRec.set('status','<div class="'+o.queue.slots[i].status+'">'+o.queue.slots[i].status+'</div>');

                            DownloadsList.add(tmpRec);
...
//                            buttons += '<img style="cursor: pointer;" onclick="nzbretry(\''+o.history.slots[count].nzo_id+'\')" src="../static/images/retry.png">';
